 ...
 *       console.log('documentsxxxxxxx with 0 tags', docs);
 *     })
 *
...
/**
 * Specifies the maximum number of documentsxxxxxxx the query will return.
 *
 * ####Example
 *
 *     query.limit(20)
...
/**
 * Specifies the number of documentsxxxxxxx to skip.
 *
 * ####Example
 *
 ...
 *
 * When no `callback` is passed, the query is not executed. When the query is executed, the result will be an array of documentsxxxxxxx.
 *
 * ####Example
 *
...
Query.prototype.find = function (conditions, callback) {
  ...
  if (this.schema) {
    promise = new Promise(function(error, docs) {
      ...
      _this.schema.s.hooks.execPost('find', _this, [docs], callback);
    });
  } else {
  ...
  if (this.schema) {
    this.schema.s.hooks.execPre('find', this, function(error) {
      if (error) {
        return cb(error);
      }
...
/*!
 * hydrates many documentsxxxxxxx
 *
 * @param {Model} model
 * @param {Array} docs
 * @param {Object} fields
...
Query.prototype.findOne = function (conditions, fields, options, callback) {
  ...
  if (this.schema) {
    promise = new Promise(function(error, doc) {
      ...
      self.schema.s.hooks.execPost('findOne', self, [doc], function(error, doc) {
        if (error) {
          return callback(error);
        }
  ...
  this.schema.s.hooks.execPre('findOne', this, function(error) {
    if (error) {
...
Query.prototype.exec = function exec (op, callback) {
  ...
  if (this.schema && this.schema.s.queryHooks[this.op]) {
    this.schema.s.hooks.execPre(op, this, function(error) {
      if (error) {
        return promise.reject(error);
      }
      Query.base.exec.call(_this, op, function(error, results) {
        if (error) {
          return promise.reject(error);
        }

        _this.schema.s.hooks.execPost(op, _this, [results], function(error, results) {
          if (error) {
            return promise.reject(error);
          }
...
Query.prototype._castUpdateVal = function _castUpdateVal (schema, val, op, $conditional) {
  ...
  if (cond) {
    ...
    // Ensures embedded documentsxxxxxxx get ObjectIds etc.
...
/**
 * Specifies paths which should be populated with other documentsxxxxxxx.
 *
 * ####Example:
 *
 ...
 * ####Example
 *
 *     // JSON.stringify all documentsxxxxxxx before emitting
 ...
 *
 * These operators return documentsxxxxxxx sorted by distance.
 *
 * ####Example
 *
