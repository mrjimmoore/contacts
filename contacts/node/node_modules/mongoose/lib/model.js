 ...
 * top level (non-sub) documentsxxxxxxx.
 */
...
Model.prototype.$__handleSave = function $__handleSave(options) {
  ...
  if (this.isNew) {
    ...
    if (!utils.object.hasOwnProperty(obj || {}, '_id')) {
      // documentsxxxxxxx must have an _id else mongoose won't know
...
function checkDivergentArray (doc, path, array) {
  ...
  // documentsxxxxxxx from being returned (match, skip, limit) or they
...
/**
 * Signal that we desire an increment of this documentsxxxxxxx version.
 *
 * ####Example:
 *
...
Model.init = function init () {
  if ((this.schema.options.autoIndex) || (this.schema.options.autoIndex === null && this.db.config.autoIndex)){
    this.ensureIndexes();
  }
...
/**
 * Removes documentsxxxxxxx from the collection.
 *
 * ####Example:
 *
 ...
 * ####Note:
 *
 * To remove documentsxxxxxxx without waiting for a response from MongoDB, do not pass a `callback`, then call `exec` on the returned [Query](#query-js):
 *
 ...
 * ####Note:
 *
 * This method sends a remove command directly to MongoDB, no Mongoose documentsxxxxxxx are involved. Because no Mongoose documentsxxxxxxx are involved, _no middleware (hooks) are executed_.
 *
 ...
 *
 *     // passing options (in this case return the raw js objects, not mongoose documentsxxxxxxx by passing `lean`
...
/**
 * Counts number of matching documentsxxxxxxx in a database collection.
 *
 * ####Example:
 *
...
/**
 * Issues a mongodb findAndModify update command by a documentsxxxxxxx id.
 *
 * Finds a matching document, updates it according to the `update` arg, passing any `options`, and returns the found document (if any) to the callback. The query executes immediately if `callback` is passed else a Query object is returned.
 *
 * ####Options:
...
/**
 * Issue a mongodb findAndModify remove command by a documentsxxxxxxx id.
 *
 * Finds a matching document, removes it, passing the found document (if any) to the callback.
 *
 * Executes immediately if `callback` is passed, else a `Query` object is returned.
...
/**
 * Updates documentsxxxxxxx in the database without returning them.
 *
 * ####Examples:
 *
 ...
 *       if (err) return handleError(err);
 *       console.log('The number of updated documentsxxxxxxx was %d', numberAffected);
 ...
 *  - `multi` (boolean) whether multiple documentsxxxxxxx should be updated (false)
 ...
 * - `numberAffected` is the count of updated documentsxxxxxxx Mongo reported
 ...
 *
 * This helps prevent accidentally overwriting all documentsxxxxxxx in your collection with `{ name: 'jason borne' }`.
 *
 * ####Note:
 *
 ...
 * ####Note:
 *
 * To update documentsxxxxxxx without waiting for a response from MongoDB, do not pass a `callback`, then call `exec` on the returned [Query](#query-js):
 *
 ...
 * - `limit` {Number} max number of documentsxxxxxxx
 ...
 * ####NOTE:
 *
 * - Arguments are not cast to the model's schema because `$project` operators allow redefining the "shape" of the documentsxxxxxxx at any stage of the pipeline, which may leave documentsxxxxxxx in an incompatible format.
 * - The documentsxxxxxxx returned are plain javascript objects, not mongoose documentsxxxxxxx (since any shape of document can be returned).
 ...
 *     })
 *
 *     // imagine a Weapon model exists with two saved documentsxxxxxxx:
 *     //   { _id: 389, name: 'whip' }
 ...
 *
 * @param {Document|Array} docs Either a single document or array of documentsxxxxxxx to populate.
...
/*!
 * Assigns documentsxxxxxxx returned from a population query back
 * to the original document path.
 */

function assignVals (o) {
  // replace the original ids in our intermediate _ids structure
  // with the documentsxxxxxxx found by query

  assignRawDocsToIdStructure(o.rawIds, o.rawDocs, o.rawOrder, o.options);

  // now update the original documentsxxxxxxx being populated using the
  // result structure that contains real documentsxxxxxxx.
...
/*!
 * 1) Apply backwards compatible find/findOne behavior to sub documentsxxxxxxx
 *
 *    find logic:
 *      a) filter out non-documentsxxxxxxx
 ...
 *   else documentsxxxxxxx are put back in original order of array if found in results
 *
 * @param {Array} rawIds
 * @param {Array} vals
 * @param {Boolean} sort
...
function assignRawDocsToIdStructure (rawIds, resultDocs, resultOrder, options, recursed) {
  ...
  for (var i = 0; i < rawIds.length; ++i) {
    ...
    if (recursed) {
      ...
      // assign matching documentsxxxxxxx in original order unless sorting
      doc = resultDocs[sid];
      if (doc) {
        if (sorting) {
  ...
  rawIds.length = 0;
  if (newOrder.length) {
    // reassign the documentsxxxxxxx based on corrected order
...
Model.compile = function compile (name, schema, collectionName, connection, base) {
  ...
  if (versioningEnabled && !schema.paths[schema.options.versionKey]) {
    // add versioning to top level documentsxxxxxxx only
    var o = {};
