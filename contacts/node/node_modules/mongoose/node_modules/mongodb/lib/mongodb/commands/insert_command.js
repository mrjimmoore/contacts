...
var InsertCommand = exports.InsertCommand = function(db, collectionName, checkKeys, options) {
  ...
  if(options['keepGoing'] != null && options['keepGoing']) {
    // This will finish inserting all non-index violating documentsxxxxxxx even if it returns an error
    this.flags = 1;
  }
...
struct {
    ...
    BSON[]    documentsxxxxxxx;          // one or more documentsxxxxxxx to insert into the collection
}
*/
InsertCommand.prototype.toBinary = function(bsonSettings) {
  ...
  // Write all the bson documentsxxxxxxx to the buffer at the index offset
  for(var i = 0; i < this.documents.length; i++) {
