...
/**
 * Inserts a single document or a an array of documentsxxxxxxx into MongoDB.
 *
 * Options
 ...
 *  - **continueOnError/keepGoing** {Boolean, default:false}, keep inserting documentsxxxxxxx even if one document has an error, *mongodb 1.9.1 >*.
 *  - **serializeFunctions** {Boolean, default:false}, serialize functions on the document.
...
/**
 * Removes documentsxxxxxxx specified by `selector` from the db.
 *
 * Options
 *  - **w**, {Number/String, > -1 || 'majority' || tag name} the write concern for the operation where < 1 is no acknowlegement of write and w >= 1, w = 'majority' or tag acknowledges the write
 *  - **wtimeout**, {Number, 0} set the timeout for waiting for write concern to finish (combines with w option)
 ...
 *  - **fullResult** {Boolean, default:false}, returns the full result document (document returned will differ by server version)
 *
 * @param {Object} [selector] optional select, no selector is equivalent to removing all documentsxxxxxxx.
 ...
 *  - **multi** {Boolean, default:false}, update all documentsxxxxxxx matching the selector.
 *  - **serializeFunctions** {Boolean, default:false}, serialize functions on the document.
 *  - **checkKeys** {Boolean, default:true}, allows for disabling of document key checking (WARNING OPENS YOU UP TO INJECTION ATTACKS)
 *  - **fullResult** {Boolean, default:false}, returns the full result document (document returned will differ by server version)
 *
 * @param {Object} selector the query to select the document/documentsxxxxxxx to be updated
...
/**
 * Count number of matching documentsxxxxxxx in the db to a query.
 *
 * Options
 *  - **skip** {Number}, The number of documentsxxxxxxx to skip for the count.
 *  - **limit** {Number}, The limit of documentsxxxxxxx to count.
 *  - **readPreference** {String}, the preferred read preference, require('mongodb').ReadPreference (ReadPreference.PRIMARY, ReadPreference.PRIMARY_PREFERRED, ReadPreference.SECONDARY, ReadPreference.SECONDARY_PREFERRED, ReadPreference.NEAREST).
 *
 ...
 *
 * Options
 *  - **limit** {Number, default:0}, sets the limit of documentsxxxxxxx returned in the query.
 *  - **sort** {Array | Object}, set to sort the documentsxxxxxxx coming back from the query. Array of indexes, [['a', 1]] etc.
 *  - **fields** {Object}, the fields to return in the query. Object of fields to include or exclude (not both), {'a':1}
 *  - **skip** {Number, default:0}, set to skip N documentsxxxxxxx ahead in your query (useful for pagination).
 ...
 *  - **exhaust** {Boolean, default:false} have the server send all the documentsxxxxxxx at once as getMore packets, not recommended.
 ...
 *  - **raw** {Boolean, default:false}, Return all BSON documentsxxxxxxx as Raw Buffer documentsxxxxxxx.
 *  - **readPreference** {String}, the preferred read preference, require('mongodb').ReadPreference ((ReadPreference.PRIMARY, ReadPreference.PRIMARY_PREFERRED, ReadPreference.SECONDARY, ReadPreference.SECONDARY_PREFERRED, ReadPreference.NEAREST).
